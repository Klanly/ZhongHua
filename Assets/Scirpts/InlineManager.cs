...
public class InlineManager : MonoBehaviour {
    ...
    void Initialize()
    {
        ...
        {
            SpriteAsset _spriteAsset = _spriteGraphic[i].m_spriteAsset;   //获取.asset文件信息


            if (!_IndexSpriteGraphic.ContainsKey(_spriteAsset.ID)&&!_IndexSpriteInfo.ContainsKey(_spriteAsset.ID))  //判断其中有没有.asset的id  目前0为静态的  1 为动态的
            {
